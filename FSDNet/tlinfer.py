...
to_test = {'ViSha': '/nfs/bigcornea/add_disk0/shilinhu/SBUTimeLapse'}
...
def main():
    ...
    if len(args['snapshot']) > 0:
        ...
        net.load_state_dict(torch.load(os.path.join(ckpt_path, 'FSDNetvisha', args['snapshot'] + '.pth'),
    ...
    with torch.no_grad():
        for name, root in to_test.items():
            ...
            video_list = [video for video in os.listdir(video_root) if os.path.isdir(os.path.join(video_root, video))]
            img_name = []
            for v in video_list:
                for img in os.listdir(os.path.join(video_root, v)):
                    if img.endswith('.png'):
                        img_name.append(os.path.join(video_root, v, img))
            ...
            for idx, image_name in enumerate(img_name):
                ...
                img = Image.open(os.path.join(root, image_name)).convert('RGB')
