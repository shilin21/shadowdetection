...
class ESD1Model(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        Returns:
            the modified parser.
        """
        parser.set_defaults(dataset_mode='energy', lr=0.005, batch_size=8, preprocess='resize', load_size=512, no_epoch=True, save_by_iter=True, load_iter=50000, print_freq=1, display_ncols=4)
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['b1', 'b2', 'combine']
        # specify the images you want to save and display. The program will call base_model.get_current_visuals to save and display these images.
        self.visual_names = ['data_A', 'data_B', 'output1', 'output3']
        # specify the models you want to save to the disk. The program will call base_model.save_networks and base_model.load_networks to save and load networks.
        # you can use opt.isTrain to specify different behaviors for training and test. For example, some networks will not be used during test, and you don't need to load them.
        self.model_names = ['ESD']
        # define networks; you can use opt.isTrain to specify different behaviors for training and test.
        self.netESD = networks.define_esd(opt.backbone, opt.output_stride, opt.num_classes, opt.sync_bn, opt.freeze_bn, gpu_ids=self.gpu_ids)
        ...
        if self.isTrain:  # only defined during training time
            ...
            self.train_params = [{'params': self.netESD.module.get_1x_lr_params(), 'lr': opt.lr},
                    {'params': self.netESD.module.get_10x_lr_params(), 'lr': opt.lr * 10}]
    ...
    def forward(self):
        ...
        self.output1, self.output3 = self.netESD(self.data_A)  # generate output image given the input data_A

    def backward(self):
        """Calculate losses, gradients, and update network weights; called in every training iteration"""
        # calculate loss given the input and intermediate results
        self.loss_b1 = self.criterionLoss(self.output1, self.data_B)
        self.loss_b2 = self.criterionLoss(self.output3, self.data_B)
        self.loss_combine = self.loss_b1 + self.loss_b2
