...
class UnlabelVideoDataset(BaseDataset):
    ...
    def __init__(self, opt):
        ...
        for folder in os.listdir(self.root):
            if os.path.isdir(os.path.join(self.root, folder)):
                images = [os.path.join(folder, img_name) for img_name in os.listdir(os.path.join(self.root, folder)) if img_name.endswith('.png')]
    ...
    def __getitem__(self, index):
        ...
        # just to fit the requirement of network input, does not mean anything
        gt1 = Image.open(os.path.join(self.root, self.imgs[idx])).convert('L')
        fir_gt = i_transform(gt1)
        gt2 = Image.open(os.path.join(self.root, self.imgs[idx+1])).convert('L')
